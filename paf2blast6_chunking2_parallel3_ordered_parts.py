...
#There has to be a faster way to do all this. Looped python calcs are slow.
...
class QualityCalculations:
	def __init__(self, genome_size=5_000_000_000):
		self._k = 0.1
		self._lambda = 1.58
...
def parallel_process_file(input_file, odir, chunk_size=100_000, max_lines=5_000_000, n_threads=5,tdir=''):
	...
	output_file = os.path.join(odir, os.path.basename(input_file.replace(".paf", ".out")))
	...
	for temp_file in temp_files_sorted:
		...
		last_files_qnames = this_file_qnames
