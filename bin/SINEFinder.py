...
def options(parser):
	...
	parser.add_argument(
		'--runtype',
		choices=['chunkwise', 'seqwise'],
		default='chunkwise',
		help='Run type (default: chunkwise)'
	)
	...
	parser.add_argument(
		...
		type=int,
		default=2,
		help='TSD mismatch tolerance (default: 2)'
	)
	...
	parser.add_argument(
		...
		type=int,
		default=5,
		help='TSD minimum word size (default: 5)'
	)
	...
	parser.add_argument(
		...
		type=int,
		default=1,
		help='TSD mismatch penalty (default: 1)'
	)
	...
	parser.add_argument(
		...
		type=int,
		default=10,
		help='TSD score cutoff (default: 10)'
	)
	...
	parser.add_argument(
		...
		choices=['F', 'R', 'FR'],
		default='FR',
		help='TSD orientation (default: FR)'
	)
	...
	parser.add_argument(
		...
		type=int,
		default=1000,
		help='Overlap size (default: 1000)'
	)
	...
	parser.add_argument(
		...
		choices=['fasta'],
		default='fasta',
		help='Output type (default: fasta)'
	)
...
options = {
	...
	'TSD_SCORE_CUTOFF': 10,
	'TSD_ORIENTATION': 'F',  # possible: 'F' 'R' 'FR':
	# 'R' equal to 'F' but as
...
#Need to figure out the sfargs argument
def search_init(seqfile, sfargs):
	global fa
	global sf
	fa = pyfastx.Fasta(seqfile)
	sf = SINEFinder(**sfargs)
	
def search_chunk(args):
	seqid, start, end = args
	sequence = fa.fetch(seqid, (start, end,))
	sf.set_sequence(seqid, sequence, start - 1)
	sf.search('FR')
	sf.evaluate_TSD()
	res = sf.matches_as_fasta(['TSD_1', 'a_box', 'b_box', 'polyA', 'TSD_2'])
	
	return seqid, res

#Add other options? Might not be needed
def run_parallel(seqfile, outfile, chunksize = 10_000, overlap = 1_000, threads = 1):
	#Create pyfastx index for rapid random access to sequences
	if not os.path.exists(f'{seqfile}.fxi'):
		print('Indexing sequences for search')
	fa = pyfastx.Fasta(seqfile, build_index = True)
	
	#Pyfastx indices are actually sqlite3 databases
	#The API doesn't expose sequence length by default, but the database has it
	conn = sqlite3.connect(f'{seqfile}.fxi')
	curs = conn.cursor()
	seqlens = curs.execute('SELECT chrom, blen FROM seq').fetchall()
	curs.close()
	conn.close()
	seqlens = {seqid:seqlen for seqid, seqlen in seqlens}
	
	#Plan chunks of no more than --chunksize with --overlap bp overlap
	args = []
	#Pyfastx fetch is one-indexed, so we set up the correct indices here
	for s in seqlens:
		total_seqlen = seqlens[s]
		offset = 0
		while offset < total_seqlen:
			next_chunk = (s, offset+1, offset + 1 + chunksize,)
			offset += (chunksize - overlap)
			#Final group capture is slightly short, but that's OK
			if offset >= total_seqlen:
				next_chunk = (s, offset + 1 - chunksize + overlap, total_seqlen)
			args.append(next_chunk)
		
	#Will be roughly genome_size_in_bp / chunksize pieces.
	#print(len(args))
	
	#Ensure not more threads than processing pieces - extremely, extremely unlikely for this code
	ok_procs = min([len(args), threads])
	
	ct = len(args)
	with multiprocessing.Pool(ok_procs, initializer = search_init, initargs = (seqfile, options,)) as pool:
		with open(outfile, 'w') as out:
			for id, writeout in pool.imap_unordered(search_chunk, args):
				out.write(writeout)
				#print(f'{ct} remain')
