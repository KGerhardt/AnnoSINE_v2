...
class alignment_tsd_tir_finder:
	...
	def check_polyAT(self, candidates):
		poly_at_record = []
		for c in candidates:
			query = c[0]
			target = c[1]
			...
			qgc = qctr['G'] + qctr['g'] + qctr['C']+ qctr['c']
			tgc = tctr['G'] + tctr['g'] + tctr['C']+ tctr['c']
			
			not_polyAT = qgc > self.polyAT_threshold and tgc > self.polyAT_threshold
			
			poly_at_record.append(not_polyAT)
			
		return poly_at_record
	...
	def find_longest_shared_subsequences(self, l, r, desc, minimum_length = 5):
		...
		if len(shared_substrings) > 0:
						...
						if this_repeat in es:
							for loc in existing_strings[es]:
								...
								if np.all(current_row == removal_sub):
									if desc.startswith('chr1_pat R 27320423:27320834'):	
										print('Break')
									keep = False
									break
				if keep:
			...
			while old_subs != shared_substrings.shape[0]:
				...
				if desc.startswith('chr1_pat R 27320423:27320834'):
					print('post')
					for row in shared_substrings:
						print(row)
						print(l[row[0]:row[0]+row[2]])
						print(r[row[1]:row[1]+row[2]])
						print('#######')
			'''
		else:
				...
				if rle_array.shape[0] == 1:
					...
					if rle_array.shape[0] == 3:
						...
						for c in candidate_runs:
							...
							score = np.sum(c[:,2]) - mismatch - (mismatch**2)
							if score > best_score:
								best_score = score
								winning_candidate = c
						...
						#print(offset_from_exact_left)
						#print(offset_from_exact_right)
						#print('')
...
def main():
	...
	if opts.sequences is None:
		...
		for seq in fa:
			...
			shared_substrings = mn.find_longest_shared_subsequences(l, r,  seq.description, minimum_length = 5)
			...
			if shared_substrings is not None:
				...
				#Extension routinely causes adjacent matching strings to extend into one another
				winners = mn.score_extensions(extension_strings)
				
				if opts.poly_at_ok:
					not_polyAT = [True] * len(winners)
				else:
					not_polyAT = mn.check_polyAT(winners)
				
				for sh, e, w, p in zip(shared_substrings, extension_loci, winners, not_polyAT):
					#Exclude polyAT
					if p:
						print(sh)
						print(e)
