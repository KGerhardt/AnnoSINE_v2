...
def hmm_predict(genome_assembly_path):
    dir_hmm = os.listdir(script_dir+'/../Family_Seq/')
    os.system('mkdir '+work_dir+'/HMM_out > /dev/null 2>&1')
    for num_dir_hmm in range(len(dir_hmm)):
        if dir_hmm[num_dir_hmm] != '.DS_Store':
            # Clear the content of exist output
            if os.path.exists(work_dir+'/HMM_out/' + dir_hmm[num_dir_hmm] + '.out'):
                clear_filename =work_dir+ '/HMM_out/' + dir_hmm[num_dir_hmm] + '.out'
                with open(clear_filename, "r+") as clear_f:
                    clear_f.seek(0)
                    clear_f.truncate()
            
            os.system(
                'nhmmer -o '+work_dir+'/HMM_out/' + dir_hmm[num_dir_hmm] + '.out  '
                +script_dir+ '/../Family_Seq/' + dir_hmm[num_dir_hmm] + '/' + dir_hmm[num_dir_hmm] + '.hmm '
                + genome_assembly_path)
            '''
            print('nhmmer -o '+work_dir+'/HMM_out/' + dir_hmm[num_dir_hmm] + '/'+ dir_hmm[num_dir_hmm] + '.out  '+script_dir+ '/../Family_Seq/' + dir_hmm[num_dir_hmm] + '/' +dir_hmm[num_dir_hmm] + '.hmm '+genome_assembly_path)
            exit()
            '''
    #exit()
...
def process_hmm_output_2(threshold_hmm_e_value):
    ...
    family_name = []
    update_hmm_record = []
    dir_file = glob.glob(work_dir+'/HMM_out/*.out')
    for a in range(len(dir_file)):
        if dir_file[a] != '.DS_Store':
            list_pre = process_hmm_output_1(dir_file[a], threshold_hmm_e_value)[0]
...
def search_tsd(out_genome_assembly_path):
    os.system('node '+script_dir+'/TSD_Searcher.js ' + out_genome_assembly_path)
...
def blast_rna(out_genome_assembly_path):
    os.system('blastn -query '+out_genome_assembly_path+'/Step4_rna_input.fasta '
              '-subject '+script_dir+'/../Input_Files/rna_database.fa '
...
def process_rna(out_genome_assembly_path):
    rna_database = []
    with open(script_dir+'/../Input_Files/rna_database.fa') as f:
        num = 0
        lines = f.readlines()
        for line in lines:
            if line[0] == '>':
...
def sine_finder(genome_assembly_path):
    #main_func()
    os.system('python3 '+script_dir+'/SINEFinder.py ' + genome_assembly_path)
...
def main_function():
    ...
    le=re.split('\.',input_genome_assembly_path)[-1]
    input_sine_finder = input_genome_assembly_path.replace('.'+le, '')+'-matches.fasta'
