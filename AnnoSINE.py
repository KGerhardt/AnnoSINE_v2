...
def main_function():
    ...
    if input_pattern == 1:
        ...
        if check_finished('Step1_process_hmm',[output_genome_assembly_path+'/Step1_extend_tsd_input_1.fa']):
            t1=time.time()
        ...
        if check_finished('Step1_sine_part',[output_genome_assembly_path+'/Step1_extend_tsd_input_2.fa']):
            t1=time.time()
    ...
    if check_finished('Step1_merge_part',[output_genome_assembly_path+'/Step1_extend_tsd_input.fa']):
        t1=time.time()
    ...
    t1=time.time()
    if check_finished('Step2_search_tsd',[output_genome_assembly_path+'/Step2_tsd.txt']):
        search_tsd(output_genome_assembly_path, script_dir)
    if check_finished('Step2_process_tsd',[output_genome_assembly_path+'/Step2_tsd_output.fa']):
    ...
    t1=time.time()
    if check_finished('Step3_MSA',[output_genome_assembly_path+'/Step3_blast_output.out']):
        multiple_sequence_alignment(input_blast_e_value, input_genome_assembly_path, output_genome_assembly_path,cpus)
    if check_finished('Step3_process_MSA',[output_genome_assembly_path+'/Step3_blast_process_output.fa']):
    ...
    if check_finished('Step3_process_MSA_p2',[output_genome_assembly_path+'/Step4_rna_input.fasta']):
    ...
    t1=time.time()
    if check_finished('Step4_blast_rna',[output_genome_assembly_path+'/Step4_rna_output.out']):
        blast_rna(output_genome_assembly_path, cpus, script_dir)
    if check_finished('Step4_process_blast_rna',[output_genome_assembly_path+'/Step4_rna_output.fasta']):
    ...
    t1=time.time()
    if check_finished('Step5_trf',[work_dir+'/Step4_rna_output.fasta.2.5.7.80.10.10.2000.dat']):
        tandem_repeat_finder(output_genome_assembly_path)
    if check_finished('Step5_process_trf',[output_genome_assembly_path+'/Step5_trf_output.fasta']):
    ...
    t1=time.time()
    if check_finished('Step6_extend_seq',[output_genome_assembly_path+'/Step6_irf_input.fasta']):
        extend_seq(input_genome_assembly_path, output_genome_assembly_path)
    if check_finished('Step6_irf',[output_genome_assembly_path+'/Step6_irf_input.fasta.2.3.5.80.10.20.500000.10000.dat']):
        inverted_repeat_finder(output_genome_assembly_path, irf_path)
    if check_finished('Step6_process_irf',[output_genome_assembly_path+'/Step6_irf_output.fasta']):
    ...
    t1=time.time()
    if check_finished('Step7_cluster_seq',[output_genome_assembly_path+'/Step7_cluster_output.fasta']):
